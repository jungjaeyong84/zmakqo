...
function renderMarkdown(report = {}) {
  ...
  const lines = [
    ...
    `- active_entry_n: ${summary.active_entry_n || 0} / legacy_entry_n: ${summary.legacy_entry_n || 0} / active_family: ${renderSummaryLine(summary.active_entry_family_counts)} / legacy_family: ${renderSummaryLine(summary.legacy_entry_family_counts)}`,
    `- FEBT active eligible ${pct(summary.febt_coverage_rate_active_eligible)} (${summary.febt_active_eligible_n || 0}) / missing ${summary.febt_active_missing_n || 0} / by_family: ${renderCoverageLine(summary.febt_active_eligible_by_family)} / by_market: ${renderCoverageLine(summary.febt_active_eligible_by_market)}`,
    ...
    `- fallback_payload_missing: ${summary.entry_fallback_payload_missing_n || 0} / linked_exec ${summary.entry_fallback_payload_missing_linked_n || 0}`,
    `- fallback_payload_missing_cause: ${renderSummaryLine(summary.entry_fallback_payload_missing_by_cause)} / market: ${renderSummaryLine(summary.entry_fallback_payload_missing_by_market)} / family: ${renderSummaryLine(summary.entry_fallback_payload_missing_by_family)}`,
    ...
    `- fallback_pending_active: ${summary.entry_fallback_pending_active_n || 0} / market: ${renderSummaryLine(summary.entry_fallback_pending_active_by_market)} / event: ${renderSummaryLine(summary.entry_fallback_pending_active_by_event)} / family: ${renderSummaryLine(summary.entry_fallback_pending_active_by_family)}`,
    `- fallback_pending_legacy: ${summary.entry_fallback_pending_legacy_n || 0} / family: ${renderSummaryLine(summary.entry_fallback_pending_legacy_by_family)}`,
    ...
    `- FEBT eligible coverage by canonical event: ${renderCoverageLine(summary.febt_eligible_by_canonical_event)}`,
    `- FEBT active coverage by canonical event: ${renderCoverageLine(summary.febt_active_eligible_by_event)}`,
    `- FEBT active low coverage events: ${renderCoverageLine(summary.febt_active_low_coverage_events)}`,
    `- FEBT active coverage gap: ${summary.febt_active_coverage_gap_by_event ? `${summary.febt_active_coverage_gap_by_event.low_key} ${pct(summary.febt_active_coverage_gap_by_event.low_coverage_rate)} vs ${summary.febt_active_coverage_gap_by_event.high_key} ${pct(summary.febt_active_coverage_gap_by_event.high_coverage_rate)} (gap ${pct(summary.febt_active_coverage_gap_by_event.coverage_gap)})` : "none"}`,
