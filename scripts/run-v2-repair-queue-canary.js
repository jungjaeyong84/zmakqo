...
const { runRepairQueueCanary } = require("../src/v2/repairQueueCanary");

const OUTPUT_FILENAME = "v2-repair-queue-canary.json";
...
function resolveOutputFilename(env = process.env) {
  return trimOrNull(env.DONBEOLJA_V2_REPAIR_CANARY_ARTIFACT_FILE) || OUTPUT_FILENAME;
}

async function run(env = process.env, {
  recordedAt = null,
  runRepairQueueCanaryFn = runRepairQueueCanary,
} = {}) {
  const artifactDir = resolveArtifactDir(env);
  const outputFilename = resolveOutputFilename(env);
  ensureDir(artifactDir);

  const canary = await runRepairQueueCanaryFn({
    env,
    recordedAt,
  });
  const output = Object.freeze({
    ...
    reason: canary.ok === true
      ? "V2_REPAIR_QUEUE_CANARY_HEALTHY"
      : "V2_REPAIR_QUEUE_CANARY_FAILED",
    ...canary,
  });
...
async function main(env = process.env) {
  ...
  try {
    ...
    console.error(JSON.stringify({
      ok: false,
      reason: "V2_REPAIR_QUEUE_CANARY_THROWN",
      error: {
...
if (require.main === module) {
  main().catch((error) => {
    console.error("RUN_V2_REPAIR_QUEUE_CANARY_FAIL", error && error.stack ? error.stack : String(error));
    process.exit(1);
  });
} else {
  module.exports = {
