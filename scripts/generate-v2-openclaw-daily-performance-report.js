...
async function collectDecisionEvidenceRowsForOutcomes({ outcomes = [], db = null, env = process.env } = {}) {
  const lookupKeys = collectDecisionEvidenceLookupKeysFromOutcomes(outcomes);
  ...
  const payload = await loadDecisionEvidence({
    inputPath: decisionEvidenceInputPath,
    env,
    db,
    lookupKeys,
  }).catch(() => null);
  const rows = payload && Array.isArray(payload.decisionEvidenceRows) ? payload.decisionEvidenceRows : [];
...
if (require.main === module) {
  ...
  module.exports = {
    ...
    __test: { resolveOutputFile, resolveInputRows, collectDecisionEvidenceLookupKeysFromOutcomes },
  };
}
