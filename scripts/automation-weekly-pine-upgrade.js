...
function buildOctopusTaskPrompt({ currentQa, previousQa, currentOverall, previousOverall, deltas, previousChangeAssessment, problemSignals, candidates, baseInfo, currentPack, previousPack, weeklyGovernance, objectiveRetrospective }) {
  ...
  return [
    ...
    `Shared objective: win_rate>=${weeklyGovernance && weeklyGovernance.objectiveConfig ? weeklyGovernance.objectiveConfig.min_win_rate : 0.6}, net>0, EV>0, monthly_run_rate_krw>=${weeklyGovernance && weeklyGovernance.objectiveConfig ? weeklyGovernance.objectiveConfig.min_monthly_net_krw : 150000}`,
...
async function main() {
  ...
  const report = [
    markdownSection("(I) 베이스라인 요약", [
      ...
      `- shared objective: win>=${weeklyGovernance && weeklyGovernance.objectiveConfig ? `${(Number(weeklyGovernance.objectiveConfig.min_win_rate || 0) * 100).toFixed(0)}%` : "60%"} / net>0 / EV>0 / monthly_run_rate_krw>=${weeklyGovernance && weeklyGovernance.objectiveConfig ? weeklyGovernance.objectiveConfig.min_monthly_net_krw : 150000}`,
  ...
  const weeklyAlertResult = await sendKoreanTelegramSummary({
    ...
    sections: [
      ...
      { header: "공통 목표", lines: [
        ...
        `목표 월간 순수익 ${weeklyGovernance && weeklyGovernance.objectiveConfig ? weeklyGovernance.objectiveConfig.min_monthly_net_krw : 150000} KRW`,
