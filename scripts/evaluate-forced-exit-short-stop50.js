...
// scripts/evaluate-forced-exit-short-stop50.js — the risk-managed variant GPT-5.5
// proposed in its seventh consultation, FROZEN BEFORE its test family was fetched:
// identical to evaluate-delisting-short.js except that a +50% adverse hourly high
// stops the position out at entry x 1.5 (loss -50% minus costs, no re-entry).
// Its first test is Binance Monitoring Tag events, never looked at before.
//
...
function main() {
  ...
  for (const e of events) {
    ...
    for (let k = ei + 1; k <= exitI; k += 1) {
      ...
      if (bars[k][2] >= entry * STOP) { pnl = -(STOP - 1) - 2 * COST; liq = true; exitAt = k; break; }
    }
    if (!liq) {
