...
const LABEL = "com.jeongjaeyong.donbeolja.v2repairfirestorecanary";
...
const OUTPUT_FILE = path.join(REPO_ROOT, "ops", "daily", "v2_repair_queue_firestore_canary_launchd_latest.json");
...
function setupLaunchd({
  ...
  if (args.dryRun) {
    result.reason = "V2_REPAIR_FIRESTORE_CANARY_LAUNCHD_DRY_RUN";
    result.loaded_after = result.loaded_before;
    writeJson(fsApi, outputFile, result);
    return Object.freeze(result);
  }
  ...
  result.ok = args.enable
    ...
    : result.copied === true;
  if (!result.ok) {
    result.reason = "V2_REPAIR_FIRESTORE_CANARY_LAUNCHD_ENABLE_FAILED";
  } else if (args.enable) {
    result.reason = "V2_REPAIR_FIRESTORE_CANARY_LAUNCHD_ENABLED";
  } else {
    result.reason = "V2_REPAIR_FIRESTORE_CANARY_LAUNCHD_INSTALLED";
  }
  writeJson(fsApi, outputFile, result);
  return Object.freeze(result);
}
