...
const { getExchangeSettingsForProvider, getRiskBudgetForProvider } = require("../src/utils/exchangeSettings");
...
function parseNumberList(raw, fallback = []) {
  ...
  if (!txt) return Array.isArray(fallback) ? fallback.slice() : [];
  const out = txt
    .split(",")
...
function applyReplayScenarioFeatures(baseFeatures, scenario) {
  const features = (baseFeatures && typeof baseFeatures === "object")
    ? { ...baseFeatures }
    : {};
  if (!scenario || scenario.mode !== "rescue_add_compare") return features;
  features._replay_rescue_add_enabled = true;
  features._replay_rescue_add_size = Number.isFinite(Number(scenario.addFraction))
    ? Number(scenario.addFraction)
    : 0;
  features._replay_rescue_add_min_loss_pct = Number.isFinite(Number(scenario.minLossPct))
    ? Number(scenario.minLossPct)
    : 1.0;
  features._replay_rescue_add_max_loss_pct = Number.isFinite(Number(scenario.maxLossPct))
    ? Number(scenario.maxLossPct)
    : 1.2;
  features._replay_rescue_add_min_stop_distance_pct = Number.isFinite(Number(scenario.minStopDistancePct))
    ? Number(scenario.minStopDistancePct)
    : null;
  features._replay_rescue_add_max_adds = Number.isFinite(Number(scenario.maxAdds))
    ? Number(scenario.maxAdds)
    : 1;
  features._replay_rescue_add_allow_range = scenario.allowRange === true;
  features._replay_rescue_add_tiers = Array.isArray(scenario.allowedTiers)
    ? scenario.allowedTiers.join(",")
    : "CORE,PRE_REAL";
  features._replay_rescue_add_sides = Array.isArray(scenario.allowedSides)
    ? scenario.allowedSides.join(",")
    : "LONG,SHORT";
  features._replay_rescue_add_scenario = scenario.key || null;
  return features;
}

async function writeReplaySignals(db, { replayExchange, tf, signals, scenario }) {
  const now = nowIso();
  const batchSize = 350;
  let wrote = 0;
  for (let i = 0; i < signals.length; i += batchSize) {
    ...
    for (const row of chunk) {
      ...
      const features = applyReplayScenarioFeatures(row.features_json, scenario);
...
async function copyReplayBars({ replayExchange, tf, barsBySymbol, runIdPrefix }) {
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    for (const row of chunk) {
      ...
      const id = `${replayExchange}__${symbol}__${tf}__${ms}`;
      const ref = db.collection("bars_snapshots").doc(id);
      const payload = {
        snapshot_id: id,
        run_id: `${runIdPrefix}__BARCOPY__${symbol}__${ms}`,
        exchange: replayExchange,
        symbol,
        tf,
        bar_close_time_utc: closeUtc,
        bar_close_time_utc_ms: ms,
        ohlcv_json: {
...
function summarizeTradesBySide(trades, side) {
  const target = String(side || "").toUpperCase();
  const rows = (trades || []).filter((t) => String(t && t.position_side || "").toUpperCase() === target);
  return summarizeTrades(rows);
}

function parseTextList(raw, fallback = []) {
  const txt = String(raw || "").trim();
  if (!txt) return Array.isArray(fallback) ? fallback.slice() : [];
  const out = txt
    .split(",")
    ...
    .filter(Boolean);
  return out.length ? unique(out) : (Array.isArray(fallback) ? fallback.slice() : []);
}

function makeRescueScenario(addFraction, options = {}) {
  const pctLabel = Math.round(Number(addFraction) * 100);
  const minLossPct = Number.isFinite(Number(options.minLossPct)) ? Number(options.minLossPct) : 1.0;
  const maxLossPct = Number.isFinite(Number(options.maxLossPct)) ? Number(options.maxLossPct) : 1.2;
  const minStopDistancePct = Number.isFinite(Number(options.minStopDistancePct))
    ? Number(options.minStopDistancePct)
    : 0.25;
  const maxAdds = Number.isFinite(Number(options.maxAdds)) ? Number(options.maxAdds) : 1;
  const allowRange = options.allowRange === true;
  const allowedTiers = Array.isArray(options.allowedTiers) && options.allowedTiers.length
    ? unique(options.allowedTiers.map((x) => String(x || "").trim().toUpperCase()).filter(Boolean))
    : ["CORE", "PRE_REAL"];
  const allowedSides = Array.isArray(options.allowedSides) && options.allowedSides.length
    ? unique(options.allowedSides.map((x) => String(x || "").trim().toUpperCase()).filter(Boolean))
    : ["LONG", "SHORT"];
  return {
    key: addFraction > 0 ? `rescue_add_${pctLabel}` : "baseline_no_add",
    label: addFraction > 0 ? `rescue add ${pctLabel}%` : "baseline no add",
    mode: "rescue_add_compare",
    addFraction,
    minLossPct,
    maxLossPct,
    minStopDistancePct,
...
function buildScenarioList(rawFractions, includeBaseline = true, options = {}) {
  const sizes = Array.from(new Set((rawFractions || [])
    .map((x) => Number(x))
    .filter((x) => Number.isFinite(x))))
    .sort((a, b) => a - b);
  const out = sizes.map((x) => makeRescueScenario(x, options));
  if (includeBaseline) out.unshift(makeRescueScenario(0, options));
  return out;
}

function summarizeRescueReplay({ fills, drops, intents, trades }) {
  const rescueIntentRows = (intents || []).filter((r) => r && r.features_json && r.features_json._replay_rescue_add_applied === true);
  const rescueIntentIds = new Set(rescueIntentRows.map((r) => String(r.intent_id || r._id || "")).filter(Boolean));
  const rescueFillRows = (fills || []).filter((r) => rescueIntentIds.has(String(r.intent_id || "")));
  const rescueDropRows = (drops || []).filter((r) => {
    const code = String(r && (r.reason || r.drop_reason_code) || "").toUpperCase();
    return code.startsWith("REPLAY_RESCUE_ADD_");
  });
  const rescueEntrySignals = (drops || []).filter((r) => {
    const f = r && r.features_json;
    return !!(f && f._replay_rescue_add_enabled === true);
  });
  return {
    rescue_intents: rescueIntentRows.length,
    rescue_intents_by_status: countBy(rescueIntentRows, (r) => r.status || "UNKNOWN"),
    rescue_fills: rescueFillRows.length,
    rescue_drops: rescueDropRows.length,
    rescue_drop_reasons: countBy(rescueDropRows, (r) => r.reason || r.drop_reason_code || "UNKNOWN").slice(0, 20),
    all_drop_reasons: countBy(drops, (r) => r.reason || r.drop_reason_code || "UNKNOWN").slice(0, 20),
    total_budget_drops: (drops || []).filter((r) => {
      const code = String(r && (r.reason || r.drop_reason_code) || "").toUpperCase();
      return code.includes("TOTAL_BUDGET_EXCEEDED");
    }).length,
    trades_overall: summarizeTrades(trades),
    trades_long: summarizeTradesBySide(trades, "LONG"),
    trades_short: summarizeTradesBySide(trades, "SHORT"),
    rescue_signal_marked_drops: rescueEntrySignals.length,
  };
}
...
async function runReplayScenario({
  ...
  const replayExchange = `${exchange}_REPLAY_${scenario.key.toUpperCase()}_${runTs}`;
  const runIdPrefix = `RUN__REAL_ENGINE_REPLAY__${scenario.key.toUpperCase()}__${runTs}`;

  await copyReplayBars({ replayExchange, tf, barsBySymbol, runIdPrefix });
  await writeReplaySignals(db, { replayExchange, tf, signals: sourceSignals, scenario });
  ...
  for (const step of timeline) {
    try {
      ...
      stepFail += 1;
      console.error("[REPLAY_STEP_FAIL]", scenario.key, step.symbol, step.ms, e && e.message ? e.message : String(e));
    }
  }

  const fills = await fetchByExchange(db, "fills_paper", replayExchange, 50000);
  const drops = await fetchByExchange(db, "signals_dropped", replayExchange, 50000);
  const intents = await fetchByExchange(db, "order_intents_paper", replayExchange, 50000);
  ...
  for (const [sym, list] of bySymbolFills.entries()) {
    ...
    symbolTrades[sym] = {
      ...
      long: summarizeTradesBySide(trades, "LONG"),
      short: summarizeTradesBySide(trades, "SHORT"),
    };
  ...
  const replaySummary = summarizeRescueReplay({ fills, drops, intents, trades: allTrades });
  const summary = {
    scenario,
    scope: {
    ...
    replay: {
      ...
      trades_overall: replaySummary.trades_overall,
      trades_long: replaySummary.trades_long,
      trades_short: replaySummary.trades_short,
      rescue: {
        intents: replaySummary.rescue_intents,
        intents_by_status: replaySummary.rescue_intents_by_status,
        fills: replaySummary.rescue_fills,
        drops: replaySummary.rescue_drops,
        drop_reasons: replaySummary.rescue_drop_reasons,
        total_budget_drops: replaySummary.total_budget_drops,
      },
      drops_top: replaySummary.all_drop_reasons,
  ...
  if (!keep) {
    ...
    console.error("[REPLAY_CLEANUP]", scenario.key, JSON.stringify(cleanup));
  } else {
...
async function main() {
  const quiet = parseBool(getArg("quiet", "1"), true);
  if (quiet) {
    console.log = () => {};
    console.info = () => {};
  }
  ...
  const days = Math.max(1, Math.min(120, Math.floor(parseNum(getArg("days", "90"), 90))));
  ...
  const defaultSymbols = Array.isArray(exCfg && exCfg.markets) && exCfg.markets.length
    ? exCfg.markets
    : ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT", "XRPUSDT"];
  const symbolsArg = String(getArg("symbols", defaultSymbols.join(","))).trim();
  ...
  const maxBars = Math.max(120, Math.min(3500, Math.floor(parseNum(getArg("max_bars", "2500"), 2500))));
  ...
  const addFractions = parseNumberList(getArg("add_sizes", "0.25,0.50,0.75,1.00"), [0.25, 0.50, 0.75, 1.00]);
  const includeBaseline = parseBool(getArg("include_baseline", "1"), true);
  const minLossPct = parseNum(getArg("min_loss_pct", "1.0"), 1.0);
  const maxLossPct = parseNum(getArg("max_loss_pct", "1.2"), 1.2);
  const minStopDistancePct = parseNum(getArg("min_stop_distance_pct", "0.25"), 0.25);
  const maxAdds = Math.max(1, Math.floor(parseNum(getArg("max_adds", "1"), 1)));
  const allowRange = parseBool(getArg("allow_range", "0"), false);
  const allowedTiers = parseTextList(getArg("tiers", "CORE,PRE_REAL"), ["CORE", "PRE_REAL"]);
  const allowedSides = parseTextList(getArg("sides", "LONG,SHORT"), ["LONG", "SHORT"]);
  const scenarios = buildScenarioList(addFractions, includeBaseline, {
    minLossPct,
    maxLossPct,
    minStopDistancePct,
    maxAdds,
  ...
  const liveRiskBudget = await getRiskBudgetForProvider(exchange, 0);
  const forcedReplayTotalMax = Number(liveRiskBudget && liveRiskBudget.data && liveRiskBudget.data.total_max_krw);
  if (Number.isFinite(forcedReplayTotalMax) && forcedReplayTotalMax > 0) {
    process.env.REPLAY_FORCE_TOTAL_MAX_USDT = String(forcedReplayTotalMax);
  }
  const db = getFirestore();
  ...
  for (const scenario of scenarios) {
    console.error(`[REPLAY_SCENARIO_START] ${scenario.key} add=${scenario.addFraction}`);
    const scenarioSummary = await runReplayScenario({
      db,
      exchange,
      tf,
      strategyId,
    ...
    results.push(scenarioSummary);
    console.error(
      `[REPLAY_SCENARIO_DONE] ${scenario.key} ` +
      `net=${scenarioSummary.replay.trades_overall.total_pnl_quote} ` +
      `long=${scenarioSummary.replay.trades_long.total_pnl_quote} ` +
      `short=${scenarioSummary.replay.trades_short.total_pnl_quote} ` +
      `rescue_fills=${scenarioSummary.replay.rescue.fills}`
    );
  }

  const baseline = results.find((r) => r && r.scenario && r.scenario.key === "baseline_no_add") || null;
  const summary = {
    scope: {
      baseline_strategy_id: strategyId || null,
      strict_strategy_filter: strictStrategy,
    ...
    input: {
      ...
      rescue_config: {
        add_sizes: addFractions,
        min_loss_pct: minLossPct,
        max_loss_pct: maxLossPct,
        min_stop_distance_pct: minStopDistancePct,
        max_adds: maxAdds,
    ...
    scenarios: results.map((r) => {
      const delta = baseline ? {
        delta_overall_pnl_quote: Number((r.replay.trades_overall.total_pnl_quote - baseline.replay.trades_overall.total_pnl_quote).toFixed(4)),
        delta_long_pnl_quote: Number((r.replay.trades_long.total_pnl_quote - baseline.replay.trades_long.total_pnl_quote).toFixed(4)),
        delta_short_pnl_quote: Number((r.replay.trades_short.total_pnl_quote - baseline.replay.trades_short.total_pnl_quote).toFixed(4)),
        delta_overall_win_rate_pct: (
          Number.isFinite(r.replay.trades_overall.win_rate) && Number.isFinite(baseline.replay.trades_overall.win_rate)
            ? Number((((r.replay.trades_overall.win_rate - baseline.replay.trades_overall.win_rate) * 100)).toFixed(2))
            : null
        ),
      } : null;
      return {
