...
const TP_PCT = 3.0;
const TRAIL_PCT = 1.0;
const { base: BASELINE_SCENARIO } = require("../ops/analysis/v5602_frontier_search.json");
...
function sidePnlPct(position, price, leverage) {
  if (!position || !Number.isFinite(position.entry) || !Number.isFinite(price)) return null;
  if (position.side === "LONG") return (((price - position.entry) / position.entry) * 100 * leverage);
  if (position.side === "SHORT") return (((position.entry - price) / position.entry) * 100 * leverage);
  return null;
}

function stopDistancePct(position, price) {
  if (!position || !Number.isFinite(position.sl) || !Number.isFinite(price) || price <= 0) return null;
  if (position.side === "LONG") return ((price - position.sl) / price) * 100;
  if (position.side === "SHORT") return ((position.sl - price) / price) * 100;
  return null;
}
...
function backtest(candles, symbol, sc) {
  ...
  const signalCounts = emptySignals();
  const rescueStats = {
    rescueSignalsSeen: 0,
    rescueAdds: 0,
    rescueLongAdds: 0,
  ...
  const rescueAddEnabled = sc.rescueAddEnabled === true;
  const rescueAddSize = Number.isFinite(sc.rescueAddSize) ? sc.rescueAddSize : 0.30;
  const rescueMinLossPct = Number.isFinite(sc.rescueMinLossPct) ? sc.rescueMinLossPct : 0.40;
  const rescueMaxLossPct = Number.isFinite(sc.rescueMaxLossPct) ? sc.rescueMaxLossPct : 1.20;
  const rescueMinSlDistancePct = Number.isFinite(sc.rescueMinSlDistancePct) ? sc.rescueMinSlDistancePct : 0.35;
  const rescueAllowedTiers = new Set(
    (Array.isArray(sc.rescueAllowedTiers) ? sc.rescueAllowedTiers : ["CORE", "PRE_REAL"])
      .map((x) => String(x || "").trim().toUpperCase())
      .filter(Boolean)
  );
  const rescueAllowedSides = new Set(
    (Array.isArray(sc.rescueAllowedSides) ? sc.rescueAllowedSides : ["LONG", "SHORT"])
      .map((x) => String(x || "").trim().toUpperCase())
      .filter(Boolean)
  );
  const rescueAllowRange = sc.rescueAllowRange === true;
  ...
  for (let i = 60; i < candles.length; i += 1) {
    ...
    if (position) {
      ...
      if (exitReason) {
        ...
        const feePct = COMMISSION_PCT * ((position.entryCount || 1) + 1) * leverage;
        const pnlPct = pnlPctRaw - feePct;
        const notionalUsd = INITIAL_CAPITAL * (POSITION_SIZE_PCT / 100) * (position.units || 1);
        ...
        trades.push({
          ...
          adds: position.addCount || 0,
          units: position.units || 1,
          initialEntry: position.initialEntry || position.entry,
          avgEntry: position.entry,
          rescueUsed: (position.addCount || 0) > 0,
        });
        position = null;
      }
    }
    ...
    if (realLong && cooldownLong) { tier = "REAL"; side = "LONG"; }
    else if (realShort && cooldownShort) { tier = "REAL"; side = "SHORT"; }
    else if (preRealLongFinal && cooldownLong) { tier = "PRE_REAL"; side = "LONG"; }
    else if (preRealShort && cooldownShort) { tier = "PRE_REAL"; side = "SHORT"; }
    else if (coreLongTierAllowed && cooldownLong) { tier = "CORE"; side = "LONG"; }
    else if ((sc.suppressCoreShortTier ? false : coreShortFinal) && cooldownShort) { tier = "CORE"; side = "SHORT"; }
    else if (earlySoftLong && cooldownLong) { tier = "EARLY"; side = "LONG"; }
    else if (earlySoftShort && cooldownShort) { tier = "EARLY"; side = "SHORT"; }

    if (!tier) continue;
    const signalKey = `${tier}_${side}`;
    const entry = c.close;
    if (position) {
      const sameDirection = position.side === side;
      const currentPnlPct = sidePnlPct(position, entry, leverage);
      const lossPct = Number.isFinite(currentPnlPct) ? Math.max(0, -currentPnlPct) : null;
      const slDistancePct = stopDistancePct(position, entry);
      const canRescueAdd = rescueAddEnabled
        && sameDirection
        && rescueAllowedSides.has(position.side)
        && position.tpHit !== true
        && (position.addCount || 0) < 1
        && rescueAllowedTiers.has(tier)
        && Number.isFinite(lossPct)
        && lossPct >= rescueMinLossPct
        && lossPct <= rescueMaxLossPct
        && Number.isFinite(slDistancePct)
        && slDistancePct >= rescueMinSlDistancePct
        && (rescueAllowRange || regimeState !== "range");
      if (canRescueAdd) {
        rescueStats.rescueSignalsSeen += 1;
        rescueStats.rescueAdds += 1;
        if (position.side === "LONG") rescueStats.rescueLongAdds += 1;
        if (position.side === "SHORT") rescueStats.rescueShortAdds += 1;

        const prevUnits = Number.isFinite(position.units) ? position.units : 1;
        const nextUnits = prevUnits + rescueAddSize;
        const nextEntry = ((position.entry * prevUnits) + (entry * rescueAddSize)) / nextUnits;
        position.entry = nextEntry;
        position.units = nextUnits;
        position.entryCount = (position.entryCount || 1) + 1;
        position.addCount = (position.addCount || 0) + 1;
        position.lastAddTier = tier;
        position.lastAddTime = c.time;
        position.lastAddPrice = entry;
        if (position.side === "LONG") {
          position.sl = nextEntry * (1 - (longSlPct / 100));
          position.tp = nextEntry * (1 + (longTpPct / 100));
          if (c.high > position.highWm) position.highWm = c.high;
          if (!position.tpHit) position.trailStop = nextEntry * (1 - (position.trailPct / 100));
        } else {
          position.sl = nextEntry * (1 + (shortSlPct / 100));
          position.tp = nextEntry * (1 - (shortTpPct / 100));
          if (c.low < position.lowWm) position.lowWm = c.low;
          if (!position.tpHit) position.trailStop = nextEntry * (1 + (position.trailPct / 100));
        }
      }
      continue;
  ...
  return { trades, signalCounts, rescueStats };
}
...
function mergeRescueStats(acc, stats) {
  acc.rescueSignalsSeen += Number(stats && stats.rescueSignalsSeen || 0);
  acc.rescueAdds += Number(stats && stats.rescueAdds || 0);
  acc.rescueLongAdds += Number(stats && stats.rescueLongAdds || 0);
  acc.rescueShortAdds += Number(stats && stats.rescueShortAdds || 0);
  return acc;
}

function parseNumberListEnv(name, fallback) {
...
function parseStringSetListEnv(name, fallback) {
  const raw = String(process.env[name] || "").trim();
  if (!raw) return fallback;
  const groups = raw
    .split(";")
    .map((grp) => grp.split(",").map((x) => String(x || "").trim().toUpperCase()).filter(Boolean))
    .filter((grp) => grp.length > 0);
  return groups.length ? groups : fallback;
}

function parseBooleanListEnv(name, fallback) {
  const raw = String(process.env[name] || "").trim();
  if (!raw) return fallback;
  const vals = raw
    .split(",")
...
function runScenarioAcrossSymbols(marketData, scenario) {
  let allTrades = [];
  const combinedSignals = emptySignals();
  const combinedRescueStats = mergeRescueStats({
    rescueSignalsSeen: 0,
    rescueAdds: 0,
    rescueLongAdds: 0,
    rescueShortAdds: 0,
  ...
  for (const symbol of SYMBOLS) {
    const { trades, signalCounts, rescueStats } = backtest(marketData.get(symbol), symbol, scenario);
    allTrades = allTrades.concat(trades);
    for (const [k, v] of Object.entries(signalCounts || {})) combinedSignals[k] += Number(v || 0);
    mergeRescueStats(combinedRescueStats, rescueStats);
  }

  const overall = summarizeTrades(allTrades, combinedSignals);
  const longSignals = emptySignals();
  const shortSignals = emptySignals();
  for (const [k, v] of Object.entries(combinedSignals)) {
    if (k.endsWith("_LONG")) longSignals[k] = v;
    if (k.endsWith("_SHORT")) shortSignals[k] = v;
  }

  return {
    scenario: cloneScenario(scenario),
    overall,
    long: summarizeTrades(allTrades.filter((t) => t.side === "LONG"), longSignals),
    short: summarizeTrades(allTrades.filter((t) => t.side === "SHORT"), shortSignals),
    rescue: combinedRescueStats,
    ...(process.env.INCLUDE_TRADES === "1"
      ? {
          tradesDetail: [...allTrades]
            .sort((a, b) => (a.entryTime - b.entryTime) || (a.exitTime - b.exitTime) || String(a.symbol).localeCompare(String(b.symbol))),
        }
      : {}),
  };
}

function buildRescueScenarios() {
  const addSizes = parseNumberListEnv("RESCUE_ADD_SIZES", [0.25, 0.30, 0.35]);
  const minLosses = parseNumberListEnv("RESCUE_MIN_LOSSES", [0.4, 0.6, 0.8]);
  const maxLosses = parseNumberListEnv("RESCUE_MAX_LOSSES", [1.0, 1.2]);
  const minSlDistances = parseNumberListEnv("RESCUE_MIN_SL_DISTANCES", [0.35]);
  const tierSets = parseStringSetListEnv("RESCUE_TIER_SETS", [
    ["CORE"],
    ["CORE", "PRE_REAL"],
  ]);
  const sideSets = parseStringSetListEnv("RESCUE_SIDE_SETS", [
    ["SHORT"],
    ["LONG"],
    ["LONG", "SHORT"],
  ]);
  const allowRangeOptions = parseBooleanListEnv("RESCUE_ALLOW_RANGE_OPTIONS", [false]);
  const out = [];
  for (const addSize of addSizes) {
    for (const minLoss of minLosses) {
      for (const maxLoss of maxLosses) {
        if (maxLoss <= minLoss) continue;
        for (const minSlDistance of minSlDistances) {
          for (const tiers of tierSets) {
            for (const sides of sideSets) {
              for (const allowRange of allowRangeOptions) {
                const sc = cloneScenario(BASELINE_SCENARIO);
                sc.key = `rescue_${sides.join("-").toLowerCase()}_${tiers.join("-").toLowerCase()}_${String(addSize).replace(".", "")}_${String(minLoss).replace(".", "")}_${String(maxLoss).replace(".", "")}_sl${String(minSlDistance).replace(".", "")}_${allowRange ? "range" : "trend"}`;
                sc.desc = `rescue add ${Math.round(addSize * 100)}% | loss ${minLoss}-${maxLoss}% | sl_gap ${minSlDistance}% | tiers ${tiers.join("+")} | sides ${sides.join("+")} | ${allowRange ? "range ok" : "range off"}`;
                sc.rescueAddEnabled = true;
                sc.rescueAddSize = addSize;
                sc.rescueMinLossPct = minLoss;
                sc.rescueMaxLossPct = maxLoss;
                sc.rescueMinSlDistancePct = minSlDistance;
                sc.rescueAllowedTiers = tiers;
                sc.rescueAllowedSides = sides;
                sc.rescueAllowRange = allowRange;
                out.push(sc);
              }
            }
          }
...
function scoreResult(result, baseline) {
  const overall = result && result.overall ? result.overall : {};
  const long = result && result.long ? result.long : {};
  const baseOverall = baseline && baseline.overall ? baseline.overall : {};
  const baseLong = baseline && baseline.long ? baseline.long : {};
  return Number(
    (Number(overall.netPnl || 0))
    - (Number(overall.mddPct || 0) * 55)
    + (Number(overall.winRate || 0) * 900)
    + ((Number(long.netPnl || 0) - Number(baseLong.netPnl || 0)) * 1.2)
    + ((Number(overall.netPnl || 0) - Number(baseOverall.netPnl || 0)) * 0.4)
  );
}
...
async function main() {
  ...
  const baselineScenario = cloneScenario(BASELINE_SCENARIO);
  baselineScenario.key = "baseline_no_rescue";
  baselineScenario.desc = "v5.6.0.2 current base without rescue add";
  baselineScenario.rescueAddEnabled = false;

  const baseline = runScenarioAcrossSymbols(marketData, baselineScenario);
  const candidates = buildRescueScenarios()
    .map((scenario) => {
      const result = runScenarioAcrossSymbols(marketData, scenario);
      return {
        ...result,
        score: scoreResult(result, baseline),
        deltaOverallNet: Number(result.overall.netPnl || 0) - Number(baseline.overall.netPnl || 0),
        deltaLongNet: Number(result.long.netPnl || 0) - Number(baseline.long.netPnl || 0),
        deltaWinRate: Number(result.overall.winRate || 0) - Number(baseline.overall.winRate || 0),
        deltaMddPct: Number(result.overall.mddPct || 0) - Number(baseline.overall.mddPct || 0),
      };
    })
  ...
  const outDir = path.join(__dirname, "..", "ops", "analysis");
  fs.mkdirSync(outDir, { recursive: true });
  const jsonPath = path.join(outDir, "v5602_rescue_add_report.json");
  const mdPath = path.join(outDir, "v5602_rescue_add_report.md");
  fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

  const lines = [];
  lines.push("# v5.6.0.2 Rescue Add Report");
  lines.push("");
  lines.push(`- generated_at: ${report.generated_at}`);
  lines.push(`- symbols: ${SYMBOLS.join(", ")}`);
  lines.push(`- interval: ${INTERVAL}`);
  lines.push(`- baseline overall: trades=${baseline.overall.trades}, win=${formatPct(baseline.overall.winRate)}, net=${baseline.overall.netPnl.toFixed(2)}, pf=${baseline.overall.pf.toFixed(2)}, mdd=${baseline.overall.mddPct.toFixed(2)}%`);
  lines.push(`- baseline long: trades=${baseline.long.trades}, win=${formatPct(baseline.long.winRate)}, net=${baseline.long.netPnl.toFixed(2)}, pf=${baseline.long.pf.toFixed(2)}`);
  lines.push(`- baseline short: trades=${baseline.short.trades}, win=${formatPct(baseline.short.winRate)}, net=${baseline.short.netPnl.toFixed(2)}, pf=${baseline.short.pf.toFixed(2)}`);
  lines.push("");
  if (best) {
    lines.push("## Best Rescue Add");
    lines.push(`- ${best.scenario.desc}`);
    lines.push(`- overall: trades=${best.overall.trades}, win=${formatPct(best.overall.winRate)}, net=${best.overall.netPnl.toFixed(2)}, pf=${best.overall.pf.toFixed(2)}, mdd=${best.overall.mddPct.toFixed(2)}%`);
    lines.push(`- long: trades=${best.long.trades}, win=${formatPct(best.long.winRate)}, net=${best.long.netPnl.toFixed(2)}, pf=${best.long.pf.toFixed(2)}`);
    lines.push(`- short: trades=${best.short.trades}, win=${formatPct(best.short.winRate)}, net=${best.short.netPnl.toFixed(2)}, pf=${best.short.pf.toFixed(2)}`);
    lines.push(`- rescue: signals_seen=${best.rescue.rescueSignalsSeen}, adds=${best.rescue.rescueAdds}, long_adds=${best.rescue.rescueLongAdds}, short_adds=${best.rescue.rescueShortAdds}`);
    lines.push(`- deltas: overall_net=${best.deltaOverallNet.toFixed(2)}, long_net=${best.deltaLongNet.toFixed(2)}, win=${formatPct(best.deltaWinRate)}, mdd_delta=${best.deltaMddPct.toFixed(2)}%`);
    lines.push("");
  }
  lines.push("## Top 5");
  for (const row of candidates.slice(0, 5)) {
    lines.push(`- ${row.scenario.key}: net=${row.overall.netPnl.toFixed(2)}, win=${formatPct(row.overall.winRate)}, pf=${row.overall.pf.toFixed(2)}, mdd=${row.overall.mddPct.toFixed(2)}%, long_net=${row.long.netPnl.toFixed(2)}, adds=${row.rescue.rescueAdds}, delta_net=${row.deltaOverallNet.toFixed(2)}`);
  }
