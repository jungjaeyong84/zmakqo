...
function buildProtectionRuntimeDoc({
  ...
  return {
    ...
    native_tp1_qty_abs: toNumberOrNull(nativeTp1QtyAbs),
...
function buildMlAiSignalProposalDoc({
  ...
  return {
    ...
    entry_grade: trimOrNull(upper(entryGrade)),
    trigger_type: trimOrNull(upper(triggerType)),
    signal_criteria_profile: trimOrNull(upper(signalCriteriaProfile)),
...
function buildOpenClawExecutionAuditDoc({
  ...
  return {
    ...
    symbol: trimOrNull(upper(row.symbol)),
    side: trimOrNull(upper(row.side)),
    entry_grade: trimOrNull(upper(row.entry_grade)),
    trigger_type: trimOrNull(upper(row.trigger_type)),
    setup_type: trimOrNull(upper(row.setup_type)),
    signal_criteria_profile: trimOrNull(upper(row.signal_criteria_profile)),
    signal_score: toNumberOrNull(row.signal_score),
    bar_time_ms: toNumberOrNull(row.bar_time_ms),
