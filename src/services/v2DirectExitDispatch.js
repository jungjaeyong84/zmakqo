...
// kinds. SL/TRAIL/BE force a full close; TP_P1/TP_C is a 50% partial
// close. If both sets fire simultaneously (rare race) the safer option
// is full close — the runner half is exposed to whatever just hit SL.
function resolveCloseFraction(triggeredKinds = []) {
  ...
  const isFullCloseTrigger = set.has("SL") || set.has("TRAIL") || set.has("BE");
  const isPartialCloseTrigger = set.has("TP_P1") || set.has("TP_C");
  if (isFullCloseTrigger) return { fraction: 1.0, reason: "FULL_CLOSE_SL_OR_TRAIL_OR_BE" };
  if (isPartialCloseTrigger) return { fraction: 0.5, reason: "PARTIAL_CLOSE_TP1" };
