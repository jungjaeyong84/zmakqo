          ...
          containers: Object.freeze([
            Object.freeze({
              ...
              env: Object.freeze([
                ...
                Object.freeze({ name: "DONBEOLJA_V2_REPAIR_WRITER_LEASE_FIRESTORE_ENABLED", value: "1" }),
                Object.freeze({ name: "DONBEOLJA_V2_REPAIR_WRITER_LEASE_TTL_MS", value: "60000" }),
                Object.freeze({ name: "DONBEOLJA_V2_REPAIR_WRITER_LEASE_HEARTBEAT_MS", value: "10000" }),
                ...
                Object.freeze({ name: "DONBEOLJA_V2_OPENCLAW_DECISION_GATE_ENABLED", value: "1" }),
                Object.freeze({ name: "DONBEOLJA_V2_OPENCLAW_DECISION_GATE_MODE", value: "BLOCK_ONLY" }),
...

{
  const unsafeService = JSON.parse(JSON.stringify(serviceJson()));
  const envRow = unsafeService.spec.template.spec.containers[0].env
    .find((row) => row.name === "DONBEOLJA_V2_REPAIR_WRITER_LEASE_FIRESTORE_ENABLED");
  envRow.value = "0";
  const result = checker.runCheck({
    DONBEOLJA_V2_RUNTIME_SERVICE_JSON: JSON.stringify(unsafeService),
    TAG: "v2-fixture",
  ...
  assert.strictEqual(result.ok, false);
  assert(result.blockers.includes("RUNTIME_DISCOVERY_CANARY:DONBEOLJA_V2_REPAIR_WRITER_LEASE_FIRESTORE_ENABLED_MISMATCH"));
}
