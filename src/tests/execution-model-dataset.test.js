...
assert.strictEqual(recomputedRows[0].features.policy_capacity_profile, 'NOT_POLICY_CAPACITY');
...
assert.strictEqual(recomputedRows[0].features.stale_pos_entry_latency_profile, 'NOT_STALE_POS_ENTRY');
assert.strictEqual(recomputedRows[0].features.stale_pos_webhook_profile, 'NOT_STALE_POS_ENTRY');
assert.strictEqual(recomputedRows[0].features.webhook_execution_profile, 'WEBHOOK_SAVED_OK_MID');
assert.strictEqual(recomputedRows[0].features.webhook_bar_timing_profile, 'POST_BAR_CLOSE_MID');
...
assert.strictEqual(__test.derivePolicyCapacityProfile({ noFillReason: 'TOTAL_BUDGET_EXCEEDED', noFillReasonFamily: 'POLICY_OR_CAPACITY', features: { reason: 'IN_POSITION_SAME_DIR', action: 'ADD' } }), 'TOTAL_BUDGET_SAME_DIR_ADD');
assert.strictEqual(__test.derivePolicyCapacityProfile({ noFillReason: 'RISK_BUDGET_DISABLED', noFillReasonFamily: 'POLICY_OR_CAPACITY', features: { action: 'ENTRY' } }), 'RISK_BUDGET_ENTRY');
assert.strictEqual(__test.derivePolicyCapacityProfile({ noFillReason: null, noFillReasonFamily: 'POLICY_OR_CAPACITY', features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY', cost_shield_block_add: false } }), 'POLICY_CAPACITY_STALE_POS_ENTRY');
...
assert.strictEqual(__test.deriveStalePosEntryLatencyProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, wasFilled: true, signalToIntentMs: 15000 }), 'STALE_POS_FAST_INTENT_FILLED');
assert.strictEqual(__test.deriveStalePosEntryLatencyProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, wasFilled: true, signalToIntentMs: 90000 }), 'STALE_POS_MEDIUM_INTENT_FILLED');
assert.strictEqual(__test.deriveStalePosEntryLatencyProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, wasFilled: true, signalToIntentMs: 180000 }), 'STALE_POS_DELAYED_INTENT_FILLED');
assert.strictEqual(__test.deriveStalePosEntryLatencyProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, wasFilled: false, noFillReason: 'TOTAL_BUDGET_EXCEEDED', signalToIntentMs: 80000 }), 'STALE_POS_BLOCKED');
assert.strictEqual(__test.deriveStalePosWebhookProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, webhookDecision: 'SAVED', webhookHasImmediateProbe: true, webhookImmediateStatus: 'OK', signalToIntentMs: 180000 }), 'STALE_POS_SAVED_OK_DELAYED');
assert.strictEqual(__test.deriveStalePosWebhookProfile({ features: { reason: 'PINE_DROP_STALE_POS_TO_ENTRY' }, webhookDecision: 'SAVED', webhookHasImmediateProbe: false, webhookImmediateStatus: null, signalToIntentMs: 10000 }), 'STALE_POS_SAVED_NO_PROBE_FAST');
assert.strictEqual(__test.deriveWebhookExecutionProfile({ source: 'TV_WEBHOOK', entryScheduleProfile: 'EXEC_CURRENT_BAR', webhookDecision: 'SAVED', webhookHasImmediateProbe: true, webhookImmediateStatus: 'OK', signalToIntentMs: 15000, wasFilled: true }), 'WEBHOOK_SAVED_OK_FAST');
assert.strictEqual(__test.deriveWebhookExecutionProfile({ source: 'TV_WEBHOOK', entryScheduleProfile: 'EXEC_CURRENT_BAR', webhookDecision: 'SAVED', webhookHasImmediateProbe: false, signalToIntentMs: 15000, wasFilled: true }), 'WEBHOOK_SAVED_NO_PROBE');
assert.strictEqual(__test.deriveWebhookExecutionProfile({ source: 'TV_WEBHOOK', entryScheduleProfile: 'EXEC_CURRENT_BAR', webhookDecision: 'SAVED', webhookHasImmediateProbe: true, webhookImmediateStatus: 'OK', signalToIntentMs: -5000, wasFilled: true }), 'WEBHOOK_PRE_BAR_CLOSE_FILLED');
assert.strictEqual(__test.deriveWebhookBarTimingProfile({ source: 'TV_WEBHOOK', signalToIntentMs: -600000 }), 'PRE_BAR_CLOSE_GT_5M');
assert.strictEqual(__test.deriveWebhookBarTimingProfile({ source: 'TV_WEBHOOK', signalToIntentMs: -45000 }), 'PRE_BAR_CLOSE_30S_5M');
assert.strictEqual(__test.deriveWebhookBarTimingProfile({ source: 'TV_WEBHOOK', signalToIntentMs: -5000 }), 'PRE_BAR_CLOSE_LT_30S');
assert.strictEqual(__test.deriveWebhookBarTimingProfile({ source: 'TV_WEBHOOK', signalToIntentMs: 15000 }), 'POST_BAR_CLOSE_FAST');
