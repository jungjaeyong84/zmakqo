...
(() => {
  const report = buildSignalLineageReport({
    ...
    fills: [
      ...
      { event: "SYNC_FILL", signal_doc_id: "SIG__1", intent_id: null, exec_price_source: "BINANCE_USER_TRADES", decision_reason: "EXTERNAL_FILL_RECONCILED" },
  ...
  assert.strictEqual(report.summary.fills_24h_n, 4);
  assert.strictEqual(report.summary.fills_intent_id_null_n, 3);
  assert.strictEqual(report.summary.fills_intent_id_null_rate, 3 / 4);
  ...
  assert.strictEqual(report.summary.external_reconciled_fills_24h_n, 3);
  assert.strictEqual(report.summary.external_reconciled_fills_intent_id_null_n, 3);
  ...
  assert.ok(report.top_null_buckets.external_reconciled_fills_intent_id_null.some((row) => (
    row.key === "EXIT_EXTERNAL_SYNC|EXTERNAL_FILL_RECONCILED"
    && row.n === 1
  )));
  assert.strictEqual(require("../../scripts/report-signal-lineage-health").__test.isAuthoritativeEntryFill({
    event: "SYNC_FILL",
