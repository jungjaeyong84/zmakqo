...
LATEST_CODEX_PATCH_JSON="/Users/jeongjaeyong/Projects/donbeolja/ops/daily/codex_weekly_patch_engine_latest.json"
...
CODEX_PATCH_FRESH_WINDOW_SEC=$((6 * 60 * 60))
...
if [ ! -f "$LATEST_CODEX_PATCH_JSON" ]; then
  node scripts/automation-codex-weekly-patch-engine.js
else
  CODEX_PATCH_LATEST_MTIME="$(stat -f %m "$LATEST_CODEX_PATCH_JSON" 2>/dev/null || echo 0)"
  CODEX_PATCH_AGE_SEC=$((NOW_EPOCH - CODEX_PATCH_LATEST_MTIME))
  if [ "$CODEX_PATCH_AGE_SEC" -gt "$CODEX_PATCH_FRESH_WINDOW_SEC" ]; then
    node scripts/automation-codex-weekly-patch-engine.js
  fi
fi

node scripts/automation-weekly-pine-upgrade.js
