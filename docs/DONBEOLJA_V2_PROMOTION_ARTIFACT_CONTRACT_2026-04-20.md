...
LIVE deploy decision은 개별 artifact freshness뿐 아니라 evidence cycle consistency도 검증해야 한다. `repair_firestore_canary_streak`, `production_entry_route_canary_streak`, `exit_runtime_canary_streak`, `production_entry_protected_canary` 의 `artifact_dir` 는 모두 같은 promotion artifact cycle이어야 하며, protected-entry proof의 `route_result_summary.position_cycle_id` 는 promotion `position_cycle_id` 와 같아야 한다. 불일치 시 `DEPLOY_DECISION:LIVE_EVIDENCE_ARTIFACT_CYCLE_MISMATCH` 또는 `DEPLOY_DECISION:LIVE_PROTECTED_ENTRY_POSITION_CYCLE_MISMATCH` 로 fail-closed 한다. 이는 서로 다른 실행에서 나온 PASS 증거를 조합해 LIVE로 올리는 V1식 증거 혼합을 차단하기 위한 계약이다.
