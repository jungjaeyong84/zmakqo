...
core V2 invariant도 artifact audit 외부 가정으로 남기지 않는다. promotion CI의 `test:v2-promotion` 은 첫 단계로 `test:v2-core-invariants` 를 실행해야 하며, 이 묶음은 canonical reducer, tick worker, exit fill ingestion, watchdog repair, TP0 retirement, native protection unprotected window, legacy TP0 namespace, fill canonical lineage guard behavior tests를 포함해야 한다. 이는 token/문자열 기반 audit가 통과해도 실제 reducer/tick/fill/watchdog 불변식이 깨지는 V1식 회귀를 차단하기 위한 계약이다.
