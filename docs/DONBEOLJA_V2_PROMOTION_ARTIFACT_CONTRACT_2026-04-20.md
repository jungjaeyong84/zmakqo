...
23. LIVE mode에서는 `bounded_runtime_summary.openclaw_supreme_control_plane_summary` 가 world state hash, validated execution permit, outcome adjudication, shadow-only learner evaluation을 모두 증명해야 하며, 위반 시 `SUBMIT_CHK_23`/runbook 31로 fail-closed 된다. 이 항목은 `approval_contract.openclaw_supreme_control_plane_closed_loop_required=true` 와 `approval_evidence_sources.openclaw_supreme_control_plane_closed_loop` 로 submit request에 남아야 한다. 또한 `lineage_consistency_summary.ok=true`, `expected_openclaw_decision_id`, `expected_position_cycle_id`, `expected_world_state_hash` 가 존재하고 `permit_lineage_mismatch_n=0`, `outcome_lineage_mismatch_n=0`, `learner_lineage_mismatch_n=0` 이어야 한다. 이는 V1에서 발생한 “서로 다른 실행 증거를 조합한 PASS”를 LIVE submit 전에 차단하기 위한 필수 조건이다
