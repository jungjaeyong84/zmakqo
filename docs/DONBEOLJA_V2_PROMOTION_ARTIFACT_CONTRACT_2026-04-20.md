...
또한 protected-entry proof는 route/protection kernel만 증명해서는 부족하다. 같은 artifact 안에 `live_endpoint_probe_summary` 를 포함해야 하며, 이 요약은 no-exchange 환경에서 `DONBEOLJA_V2_PRODUCTION_ENTRY_LIVE_ENDPOINT_ENABLED=1`, `confirm=EXECUTE_V2_LIVE_ENTRY`, `decision_mode=LIVE`, transport resolution, route invocation까지 통과했음을 보여야 한다. deploy decision은 `live_endpoint_probe_summary.reason=V2_PRODUCTION_ENTRY_LIVE_EXECUTED_AND_PROTECTED`, `endpoint_enabled=true`, `route_called=true`, `transport_resolution_ok=true`, `transport_reason=V2_PRODUCTION_ENTRY_LIVE_TRANSPORTS_READY`, `exchange_write_performed=false`, `runtime_enabled=true`, `runtime_dry_run=false`, `runtime_canary_only=false` 가 아니면 `DEPLOY_DECISION:PRODUCTION_ENTRY_PROTECTED_CANARY_REQUIRED` 로 fail-closed 해야 한다. 필수 check id는 `V2_PROTECTED_ENTRY_CANARY_LIVE_ENDPOINT_PROBE_OK`, `V2_PROTECTED_ENTRY_CANARY_LIVE_ENDPOINT_ROUTE_CALLED`, `V2_PROTECTED_ENTRY_CANARY_LIVE_ENDPOINT_TRANSPORTS_READY`, `V2_PROTECTED_ENTRY_CANARY_LIVE_ENDPOINT_NO_EXCHANGE_WRITE` 다.
...
LIVE deploy decision은 개별 artifact freshness뿐 아니라 evidence cycle consistency도 검증해야 한다. `repair_firestore_canary_streak`, `production_entry_route_canary_streak`, `exit_runtime_canary_streak`, `production_entry_protected_canary` 의 `artifact_dir` 는 모두 같은 promotion artifact cycle이어야 하며, 그 값은 실제 deploy decision을 쓰는 `V2_PROMOTION_ARTIFACT_DIR` 와도 같아야 한다. 세 long-run streak의 `position_cycle_id` 는 promotion `position_cycle_id` 와 같아야 한다. protected-entry proof의 `route_result_summary.position_cycle_id` 도 promotion `position_cycle_id` 와 같아야 한다. 불일치 시 `DEPLOY_DECISION:LIVE_EVIDENCE_ARTIFACT_CYCLE_MISMATCH`, `DEPLOY_DECISION:LIVE_STREAK_POSITION_CYCLE_MISMATCH`, `DEPLOY_DECISION:LIVE_PROTECTED_ENTRY_POSITION_CYCLE_MISMATCH` 중 하나로 fail-closed 한다. 이는 서로 다른 실행에서 나온 PASS 증거를 조합하거나, 네 증거가 모두 같은 오래된 dir를 가리키는 상태로 LIVE를 여는 V1식 증거 혼합을 차단하기 위한 계약이다.
...
Long-run collector execution summary contract: `bounded_runtime_summary.production_entry_route_canary_streak.collector_execution_summary` 와 `bounded_runtime_summary.exit_runtime_canary_streak.collector_execution_summary` 는 LIVE 승인 필수 증거다. 두 요약은 `status=PASS`, `history_source=FIRESTORE`, `firestore_source_required=true`, `exchange_write_performed=false`, `blockers=[]` 를 가져야 하며, production route는 `scheduler_job_id=v2_production_entry_route_canary`, `producer_script=run-v2-production-entry-route-canary`, `producer_scope=production_entry_route_canary`, `canary_mode=NO_EXCHANGE_ROUTE_PROOF`, exit runtime은 `scheduler_job_id=v2_exit_runtime_canary`, `producer_script=run-v2-exit-runtime-canary`, `producer_scope=exit_runtime_canary`, `canary_mode=LIVE_EXIT_RUNTIME_OBSERVATION` 이어야 한다. deploy decision은 `row_n`, `healthy_run_n`, `latest_age_minutes`, `coverage_minutes`, `max_observed_gap_minutes` 가 top-level streak fields와 숫자까지 일치할 때만 승인해야 하며, 누락 또는 drift는 각각 `DEPLOY_DECISION:PRODUCTION_ENTRY_ROUTE_CANARY_STREAK_REQUIRED`, `DEPLOY_DECISION:EXIT_RUNTIME_CANARY_STREAK_REQUIRED` 로 fail-closed 한다. 이는 V1에서 artifact는 PASS였지만 실제 scheduler/producer가 깨져 장기 증거가 끊기는 유형을 차단하기 위한 계약이다.
...
exit runtime streak artifact는 상단 숫자뿐 아니라 `long_run_quality_summary` 를 포함해야 한다. 이 요약은 `status`, `history_source`, `firestore_source_required`, `coverage_minutes`, `latest_age_minutes`, `max_observed_gap_minutes`, 그리고 `defect_counts.tp1_missing_n/native_refresh_unhealthy_n/unprotected_window_violation_n/alert_silent_drop_n/alert_retry_unresolved_n/alert_outbox_integrity_gap_n` 을 한 묶음으로 보존해 운영자가 왜 LIVE가 막혔는지 raw history를 열지 않고도 판독하게 한다. LIVE deploy decision은 `long_run_quality_summary` 의 freshness/coverage/defect counters가 top-level streak fields와 숫자까지 일치할 때만 승인해야 하며, 요약 누락 또는 drift는 `DEPLOY_DECISION:EXIT_RUNTIME_CANARY_STREAK_REQUIRED` 로 fail-closed 해야 한다.
...
repair queue runtime도 artifact 외부 가정으로 남기지 않는다. promotion CI의 `test:v2-promotion` 은 `test:v2-repair-queue-runtime` 을 실행해야 하며, 이 묶음은 `v2-repair-queue-worker.test.js`, `v2-repair-queue-live-worker.test.js`, `v2-repair-queue-live-service.test.js`, `v2-repair-delegated-executor.test.js`, `check-v2-repair-queue-canary-preflight.test.js`, `run-v2-repair-queue-service.test.js` 를 포함해야 한다. 이는 보호주문 복구 코드가 존재해도 실제 service entrypoint, delegated executor, completion ledger, canary preflight가 promotion gate 밖에서 조용히 깨지는 V1식 복구 공백을 차단하기 위한 계약이다.

core V2 invariant도 artifact audit 외부 가정으로 남기지 않는다. promotion CI의 `test:v2-promotion` 은 첫 단계로 `test:v2-core-invariants` 를 실행해야 하며, 이 묶음은 canonical reducer, tick worker, exit fill ingestion, watchdog repair, Binance protection transport deadline/abort, delegated repair writer lease, TP0 retirement, native protection unprotected window, legacy TP0 namespace, fill canonical lineage guard behavior tests를 포함해야 한다. 이는 token/문자열 기반 audit가 통과해도 실제 reducer/tick/fill/watchdog/protection transport/repair lease 불변식이 깨지는 V1식 회귀를 차단하기 위한 계약이다.
...
10. `approval_contract.production_runtime_chain_audit_required`
11. `approval_contract.production_cutover_audit_required`
12. `approval_contract.production_runtime_config_contract_required`
13. `approval_contract.production_live_entry_sizing_contract_required`
14. `approval_contract.openclaw_supreme_control_plane_closed_loop_required`
15. `approval_contract.openclaw_execution_audit_ledger_write_required`
16. `approval_contract.repair_firestore_canary_streak_required`
17. `approval_contract.production_entry_route_canary_streak_required`
18. `approval_contract.exit_runtime_canary_streak_required`
19. `approval_contract.production_entry_protected_canary_required`
20. `approval_contract.production_cutover_readiness_summary_required`
21. `approval_contract.scheduler_traffic_collector_preflight_summary_required`
22. `approval_contract.scheduler_traffic_cutover_readiness_summary_required`
23. `approval_contract.live_cutover_readiness_summary_required`
24. `approval_contract.runbook_review_pass_required`
25. `approval_contract.candidate_selection_ready_required`
26. `approval_contract.selected_preflight_required`
27. `approval_contract.blocker_free_required`
28. `approval_contract.recommended_next_action_required`
29. `approval_contract.resolved_artifact_dir_required`
...
8. `approval_evidence_sources.production_runtime_chain_audit`
9. `approval_evidence_sources.production_cutover_audit`
10. `approval_evidence_sources.production_runtime_config_contract`
11. `approval_evidence_sources.production_live_entry_sizing_contract`
12. `approval_evidence_sources.openclaw_supreme_control_plane_closed_loop`
13. `approval_evidence_sources.openclaw_execution_audit_ledger_write`
14. `approval_evidence_sources.repair_firestore_canary_streak`
15. `approval_evidence_sources.production_entry_route_canary_streak`
16. `approval_evidence_sources.exit_runtime_canary_streak`
17. `approval_evidence_sources.production_entry_protected_canary`
18. `approval_evidence_sources.production_cutover_readiness_summary`
19. `approval_evidence_sources.scheduler_traffic_collector_preflight_summary`
20. `approval_evidence_sources.scheduler_traffic_cutover_readiness_summary`
21. `approval_evidence_sources.live_cutover_readiness_summary`
22. `approval_evidence_sources.runbook_review`
23. `approval_evidence_sources.recommended_next_action`
24. `approval_evidence_sources.blocker_summary`
25. `approval_evidence_sources.lineage_hash_sources`
26. `approval_evidence_sources.candidate_selection` (auto-select path만)
27. `approval_evidence_sources.resolved_artifact_dir`
...
15. bounded canary/live mode에서는 `fill_sync_canonical_boundary_audit` 이 `V2_FILL_SYNC_CANONICAL_BOUNDARY_AUDIT_PASS` 를 증명해야 하며, 위반 시 `SUBMIT_CHK_18`/runbook 25로 fail-closed 된다. `test:v2-promotion` 은 `v2-fill-sync-canonical-boundary-audit.test.js` 를 직접 실행해야 하며, 이 감사는 `V2_FILL_SYNC_LEGACY_CANONICAL_BACKFILL_EXPLICIT_ONLY` 로 V2 batch 이후 legacy canonical write가 `DONBEOLJA_FILL_SYNC_LEGACY_CANONICAL_BACKFILL_ENABLED=1` 이 켜져도 기존 canonical truth를 다시 쓰지 못하는지 검증해야 한다. explicit legacy backfill은 V2 batch ownership이 없는 과거/복구 케이스에만 허용된다
...
23. LIVE mode에서는 `bounded_runtime_summary.openclaw_supreme_control_plane_summary` 가 world state hash, non-expired validated execution permit, outcome adjudication, fresh shadow-only learner evaluation, collector provenance를 모두 증명해야 하며, 위반 시 `SUBMIT_CHK_23`/runbook 31로 fail-closed 된다. 이 항목은 `approval_contract.openclaw_supreme_control_plane_closed_loop_required=true` 와 `approval_evidence_sources.openclaw_supreme_control_plane_closed_loop` 로 submit request에 남아야 한다. 또한 `lineage_consistency_summary.ok=true`, `expected_openclaw_decision_id`, `expected_position_cycle_id`, `expected_world_state_hash`, `expected_openclaw_execution_permit_ids`, `expected_openclaw_outcome_adjudication_ids` 가 존재하고 `permit_lineage_mismatch_n=0`, `outcome_lineage_mismatch_n=0`, `learner_lineage_mismatch_n=0`, `learner_shadow_summary.stale_evaluation_n=0`, `0 < learner_shadow_summary.max_evaluation_age_minutes <= 1440`, `0 <= learner_shadow_summary.max_observed_evaluation_age_minutes <= learner_shadow_summary.max_evaluation_age_minutes`, `learner_shadow_summary.latest_evaluated_at` 존재 조건을 모두 만족해야 한다. 추가로 `collector_execution_summary.status=PASS`, `producer_script=collect-v2-promotion-runtime-snapshot`, `producer_scope=openclaw_supreme_control_plane`, `source=V2_FIRESTORE_COLLECTOR`, `artifact_filename=promotion-runtime-snapshot.json`, `artifact_current_dir_match=true`, `generated_at`/`artifact_generated_at` 존재, `artifact_generated_age_minutes <= 180`, `exchange_write_performed=false`, `collector_execution_summary.position_cycle_id=expected_position_cycle_id`, `collector_execution_summary.openclaw_decision_id=expected_openclaw_decision_id`, `collector_execution_summary.openclaw_execution_permit_ids=expected_openclaw_execution_permit_ids`, `collector_execution_summary.openclaw_outcome_adjudication_ids=expected_openclaw_outcome_adjudication_ids` 여야 한다. 또한 LIVE mode의 cycle coherence에서는 OpenClaw supreme collector artifact도 production entry route streak, exit runtime streak, repair Firestore streak, protected-entry proof와 같은 artifact dir 및 같은 promotion position cycle에 있어야 하며, streak 3종과 OpenClaw collector의 `artifact_generated_at` 은 30분 이내여야 한다. 이는 V1에서 발생한 “서로 다른 실행 증거를 조합한 PASS”, “필드는 PASS지만 오래된 shadow 평가를 재사용하는 PASS”, “freshness 측정 필드가 빠진 PASS artifact”, “collector 실행 주체가 불명확한 PASS artifact”, “decision은 같지만 permit/outcome이 다른 폐루프 증거를 조합한 PASS”를 LIVE submit 전에 차단하기 위한 필수 조건이다
...
28. V2 protection write transport는 실행 시점 deadline과 abort signal을 가져야 한다. 기본 deadline은 `DONBEOLJA_V2_PROTECTION_WRITE_DEADLINE_MS` 로 조정 가능하되, deadline 초과는 in-flight operation의 `AbortController` signal을 abort하고 `BINANCE_NATIVE_STOP_REFRESH_DEADLINE_EXCEEDED`, `BINANCE_TP1_REPAIR_DEADLINE_EXCEEDED`, `BINANCE_FULL_PROTECTION_SL_DEADLINE_EXCEEDED`, `BINANCE_FULL_PROTECTION_TP1_DEADLINE_EXCEEDED` 중 하나의 실패 ack로 ledger에 남아야 한다. 이는 사후 canary가 아니라 주문 write 시점에서 무기한 대기/무보호 구간을 fail-closed 하기 위한 계약이다.
...
36. OpenClaw decision bundle ledger is reconstructable. decision bundle hash는 decision doc에만 남는 보조 필드가 아니라 `OPENCLAW_DECISION_BUNDLES` ledger doc의 primary reconstruction key여야 한다. 이 ledger는 `openclaw_decision_bundle_hash`, `openclaw_decision_id`, `signal_intent_id`, `bundle_payload`, `canonical_json_sha256` 를 보존해야 하며, promotion collector는 ledger 누락을 `OPENCLAW_DECISION_BUNDLE_LEDGER_REQUIRED` 로 fail-closed 해야 한다. 이 계약은 `SUBMIT_CONTRACT_CHK_84` 로 검증한다.
37. Cloud Run live env exact values are enforced. LIVE scheduler collector preflight는 Cloud Run 서비스 env에서 `DONBEOLJA_V2_OPENCLAW_EXECUTION_AUDIT_LEDGER_WRITE_ENABLED=1` 과 `DONBEOLJA_V2_ALLOW_LEGACY_WEBHOOK_SIGNAL=0` 을 exact-match로 확인해야 한다. 값이 빠지거나 다르면 LIVE evidence로 인정하지 않으며, CloudBuild deploy env mapping과 production runtime config audit도 같은 값을 추적해야 한다. 이 계약은 `SUBMIT_CONTRACT_CHK_85` 로 검증한다.
...
4. `TRAIL_ACTIVATED` shadow transition은 `binanceTickExit`의 실제 native stop refresh success와 `chosen_stop_source=TRAIL`이 동시에 확인될 때만 artifact에 포함돼야 한다
   또한 exit runtime canary는 `TRAIL_ACTIVE` 상태에서 `TRAIL_ACTIVATED` transition의 `source_exchange_evidence.evidence_kind=TRAIL_ACTIVATION`, protection runtime의 `last_exchange_evidence.evidence_kind=TRAIL_ACTIVATION`, projection/runtime native stop price 일치를 모두 확인해야 한다. 누락 시 `trail_activation_evidence_gap_n` 으로 24시간 streak와 LIVE deploy decision을 fail-closed 한다. 필수 transition alert는 canonical transition, `trade_alert_outbox_v2.canonical_transition_id`, `prepared_payload.canonical_transition_id`, `delivery_request.dedupeFingerprint`, `delivery_request.dedupeKey` 가 서로 맞아야 하며, lineage 누락은 `alert_silent_drop_n`, lineage는 맞지만 `SENT`가 아닌 상태는 `alert_retry_unresolved_n`, transition별 outbox 중복/고아 outbox/position cycle drift는 `alert_outbox_integrity_gap_n` 으로 분리해 fail-closed 한다.
...
7. `_DONBEOLJA_V2_EXIT_RUNTIME_CANARY_STREAK_REQUIRE_FIRESTORE`
8. Cloud Run env:
9. `DONBEOLJA_V2_EXIT_RUNTIME_CANARY_FIRESTORE_WRITE_ENABLED`
10. `DONBEOLJA_V2_EXIT_RUNTIME_CANARY_FIRESTORE_READ_ENABLED`
11. `DONBEOLJA_V2_EXIT_RUNTIME_CANARY_STREAK_SOURCE`
12. `DONBEOLJA_V2_EXIT_RUNTIME_CANARY_STREAK_REQUIRE_FIRESTORE`
13. Submit contract check: `SUBMIT_CONTRACT_CHK_49`
