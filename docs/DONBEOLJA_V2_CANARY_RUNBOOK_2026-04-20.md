...
| 27A | `SUBMIT_CHK_20A` | `promotion-deploy-decision.json`, `v2_production_entry_protected_canary_latest.json` | `bounded_runtime_summary.production_entry_protected_canary.reason`, `route_result_summary.runtime_health_status`, `sl_order_id`, `tp1_order_id`, `exchange_write_performed` | CANARY/LIVE 필수. promotion pipeline이 현재 artifact dir에 fresh no-exchange protected canary를 생성해야 하며, `reason=V2_PRODUCTION_ENTRY_PROTECTED_CANARY_PASS`, `runtime_health_status=HEALTHY`, SL/TP1 order id 존재, `exchange_write_performed=false` 이어야 함 | route는 통과하지만 submitter/protection activation/runtime write 체인이 깨진 상태이므로 protected canary와 production route를 먼저 수정 |
