...
| 13G | `SUBMIT_CHK_04C`, `SUBMIT_CHK_11`, `SUBMIT_CHK_19`, `SUBMIT_CHK_20A`, `SUBMIT_CHK_21`, `SUBMIT_CHK_23`, `SUBMIT_CHK_06`, `SUBMIT_CHK_07` | `v2_live_evidence_readiness_latest.json`, `promotion-deploy-decision.json` | `axes[]`, `failed_axis_ids`, `submit_check_ids`, `runbook_refs`, `temporal_coherence`, `deploy_ready` | `npm run check:v2-live-evidence-readiness` 가 현재 artifact dir의 `promotion-deploy-decision.json` 을 읽고 production runtime chain, repair 24h streak, production entry route 24h streak, exit runtime 24h streak, protected entry canary, OpenClaw supreme closed loop, temporal/cycle coherence를 한 번에 PASS로 요약해야 함 | 실패한 `failed_axis_ids` 를 기준으로 해당 submit check와 runbook 번호를 먼저 수정. `temporal_coherence.ok=false` 면 artifact dir 폐기 후 fresh promotion pipeline 재실행 |
