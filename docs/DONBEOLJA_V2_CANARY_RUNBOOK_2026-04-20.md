...
| 13B | - | `promotion-cloudbuild-context.json`, `promotion-deploy-decision.json` | `deploy_decision_summary.warning_summary`, `warnings`, `final_status_line` | warning count/top warning이 서로 일치. repair streak warning은 `has_repair_firestore_canary_streak_warning=true` 와 runbook 19, production route streak warning은 `has_production_entry_route_canary_streak_warning=true` 와 runbook 26으로 추적 가능해야 함 | warning summary drift면 artifact dir 폐기 후 cloudbuild context 재생성 |
