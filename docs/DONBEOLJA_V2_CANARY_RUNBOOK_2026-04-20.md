...
12. LIVE 제출은 `bounded_runtime_summary.exit_runtime_canary_streak` 이 24시간 Firestore-backed exit runtime canary streak pass를 증명해야 하며, TP1 missing/native refresh unhealthy/unprotected window/silent alert drop 카운트가 하나라도 있으면 `SUBMIT_CHK_21`/runbook 28로 fail-closed 된다
