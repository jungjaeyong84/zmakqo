...
9. promotion pipeline은 unified report 생성 직전에 `v2_production_entry_route_canary_streak_latest.json` 를 artifact dir 안에 다시 생성하므로, submit 판단은 외부 latest 파일이 아니라 해당 승격 실행에서 갱신한 streak를 읽어야 한다
10. LIVE 제출은 `bounded_runtime_summary.production_entry_route_canary_streak.history_source=FIRESTORE` 를 요구하므로, CANARY 기간에 먼저 durable canary history를 누적해야 한다
