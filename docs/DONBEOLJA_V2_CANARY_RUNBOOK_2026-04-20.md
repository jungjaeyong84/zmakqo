...
| 30 | `SUBMIT_CHK_07` | `promotion-deploy-decision.json` | `bounded_runtime_summary.repair_firestore_canary_streak.artifact_dir`, `bounded_runtime_summary.production_entry_route_canary_streak.artifact_dir`, `bounded_runtime_summary.exit_runtime_canary_streak.artifact_dir`, `bounded_runtime_summary.production_entry_protected_canary.artifact_dir`, `bounded_runtime_summary.production_entry_protected_canary.route_result_summary.position_cycle_id` | LIVE만 필수. 장시간 canary 3종과 protected-entry proof 1종은 모두 같은 artifact cycle을 가리켜야 하며 protected-entry proof의 position cycle은 promotion position cycle과 같아야 한다. 깨지면 `DEPLOY_DECISION:LIVE_EVIDENCE_ARTIFACT_CYCLE_MISMATCH` 또는 `DEPLOY_DECISION:LIVE_PROTECTED_ENTRY_POSITION_CYCLE_MISMATCH` 로 fail-closed 된다 | 서로 다른 실행에서 나온 PASS 증거를 조합한 상태이므로 artifact dir를 폐기하고 fresh promotion pipeline을 처음부터 재실행 |
