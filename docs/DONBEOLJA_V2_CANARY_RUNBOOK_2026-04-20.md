...
| 13G | `SUBMIT_CHK_04C`, `SUBMIT_CHK_11`, `SUBMIT_CHK_19`, `SUBMIT_CHK_20A`, `SUBMIT_CHK_21`, `SUBMIT_CHK_23`, `SUBMIT_CHK_06`, `SUBMIT_CHK_07` | `v2_live_evidence_readiness_latest.json`, `promotion-cloudbuild-context.json`, `promotion-deploy-decision.json` | `live_evidence_readiness_summary`, `axes[]`, `failed_axis_ids`, `submit_check_ids`, `runbook_refs`, `temporal_coherence`, `deploy_ready` | LIVE cloudbuild wrapper가 현재 artifact dir의 `promotion-deploy-decision.json` 을 읽어 `v2_live_evidence_readiness_latest.json` 을 생성하고 context의 `live_evidence_readiness_summary` 에 보존해야 한다. production runtime chain, repair 24h streak, production entry route 24h streak, exit runtime 24h streak, protected entry canary, OpenClaw supreme closed loop, temporal/cycle coherence가 한 번에 PASS여야 함 | 실패한 `failed_axis_ids` 를 기준으로 해당 submit check와 runbook 번호를 먼저 수정. `temporal_coherence.ok=false` 면 artifact dir 폐기 후 fresh promotion pipeline 재실행 |
